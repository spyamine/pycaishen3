...
from zipline.api import schedule_function, date_rules, time_rules, order, record,symbol, history, order_target_percent
from zipline.api import set_benchmark , commission, set_commission
def initialize(context):
    """
    Called once at the start of the algorithm.
    """
    # Rebalance every day, 1 hour after market open.
    context.stocks = [symbol(x) for x in stocks if x != "MOSENEW Index"]
    context.i = 0
    set_benchmark(symbol("MOSENEW Index"))
...
def handle_data(context ,data):
    ...
    for stock in context.stocks:
        if short_mavg[i_loop]>long_mavg[i_loop]:
            order_target_percent(stock,0.01)
        elif short_mavg[i_loop]<long_mavg[i_loop]:
            order_target_percent(stock, -0.01)
        i_loop += 1
...
results = algo.run_algorithm(start = start,end = end,initialize = initialize(), handle_data = handle_data(),
                             data_frequency = data_frequency ,before_trading_start = before_trading_start(),
                             data = data,capital_base = capital_base)
